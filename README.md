...
| 用量计量 | 每次请求追加一行 `usage_log.jsonl`；管理台提供流量图表、缓存命中率与费用估算 |
| 图形管理台 | 启停、开机自启、获取令牌、健康自检、历史流量图表 |
...
缺包就自动创建 `.venv` 并安装 —— 依次尝试清华 → 阿里 → 官方三个 pip 源，任一成功即可。
（只写死单一镜像是不够的：实测本机 pip 访问清华源会被 403，而 `curl` 请求同一 URL 却是 200，
镜像的封锁是"挑客户端"的，写死一个源就会让部分机器怎么装都装不上。）
...
| 图形管理台 | `start.bat` / `启动管理台.bat` | 启停、重启、自启开关、取令牌、健康自检、流量图表 |
...
- **缓存**：大量输入 token 命中上游 prompt cache，费用统计按**命中率**计算。
...
| `clean_think_tags` | 清洗正文中残留的 `<think>` 碎片（上游切分思维链时偶发），默认 `true` |
