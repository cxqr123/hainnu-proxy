@echo off
cd /d "%~dp0"
title 一键配置到 opencode（直连学校）

rem ============================================================
rem  一键配置：把海师 DeepSeek 写进 opencode 的配置文件
rem
rem  链路：直连学校
rem   · 用之前需要：已获取登录令牌（1.获取令牌.bat）——直连会把 JWT 写进配置文件
rem  说明：
rem   · 只改 opencode 的**配置文件**，不动客户端本身，也不写死任何安装位置；
...
"%PY%" "%~dp0_setup_agent.py" --agent opencode --mode direct %*
set "RC=%ERRORLEVEL%"
...
if not "%RC%"=="0" (
  ...
  echo   完成。请按上面列出的步骤操作，然后重启 opencode。
)
echo.
pause
exit /b %RC%
